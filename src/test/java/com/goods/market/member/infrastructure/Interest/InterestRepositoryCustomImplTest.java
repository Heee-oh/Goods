...
class InterestRepositoryCustomImplTest {
    ...
    void findAllByMemberIdSupportsCursorPaging() {
        ...
        //member의 최신 순 2개만 조회
        Slice<InterestResponse> firstPage = interestRepository.findAllByMemberId(
                member.getId(),
                Long.MAX_VALUE,
                PageRequest.of(0, 2)
        );
        ...
        assertThat(firstPage.getContent().stream().map(InterestResponse::listingId))
                .doesNotContain(999L); // other꺼 들어갔는지 체크


        // 마지막 꺼낸 페이지 행 관심목록 페이지 listing id = 20L 체크
        Long cursor = firstPage.getContent().getLast().id();
        InterestResponse interestResponse = firstPage.getContent().getLast();
        Assertions.assertThat(interestResponse.listingId()).isEqualTo(20L);


        // 커서를 통해 페이지 정상작동 확인
        Slice<InterestResponse> secondPage = interestRepository.findAllByMemberId(
                member.getId(),
                cursor,
                PageRequest.of(0, 2)
        );
        ...
        assertThat(secondPage.getContent().stream().map(InterestResponse::id)) // 조회한 페이지 내 id값들이 항상 cursor보다 작은지
                .allMatch(id -> id < cursor);

        // 2페이지에는 남은 30L만 있어야 함
        assertThat(secondPage.getContent())
                .extracting(InterestResponse::listingId)
